...
| S7 | Caching: since Next 15/16 fetch defaults to no-store; personalised/auth-gated content should render dynamically, cacheable content via `use cache`/Cache Components. | Next.js caching docs; Vercel Academy | Auth-gated pages correctly `force-dynamic`. No public cacheable pages existed. | None required. Server-side pagination remains on the roadmap (currently 500-row cap fetched server-side, paginated client-side). | `src/app/(auth)/page.tsx` |
