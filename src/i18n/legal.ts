...
/*
 * The legal pages live in their own i18next namespace rather than in
 * translations.ts, and register themselves here at module scope.
 *
 * Why a separate namespace: this is roughly as much text as the entire rest
 * of the UI. translations.ts is pulled in by the root layout's I18nProvider,
 * so anything added there ships with every route; this module is imported
 * only by the two legal page components, which keeps it in their route
 * chunks. Registration happens at import time — before either component
 * renders — so the server render and the first client render both see the
 * bundle, exactly like the hot-reload re-registration in ./index.ts.
 *
 * Why the text is data and not JSX: it used to be five hand-written German,
 * English, French, Turkish and Spanish component trees behind a chain of
 * `lang.startsWith(...)` checks. Nothing stopped a locale from quietly
 * saying something different — and three of them did, abridging the
 * processor list and the data-subject rights that Art. 13 GDPR requires to
 * be identical whatever language the reader picked. One shared key set makes
 * that impossible: a locale either has the key or the parity test fails.
 *
 * Inline markup is written as tags inside the strings (<strong>, <code>,
 * <em>, <br />, and <privacy> for the cross-link) and rendered through
 * react-i18next's <Trans>, so a translator can move emphasis and links to
 * where their language needs them instead of the sentence being glued
